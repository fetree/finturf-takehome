...
Risk data is intentionally stored in a separate `business_risks` table rather than on the `businesses` table. Financials like revenue, debt, employee count, and owner credit score change over time — storing them per evaluation creates a historical snapshot so you can see how a business's profile has evolved across evaluations, not just its current state. It's an audit log, something essential especially when dealing with KYB.
